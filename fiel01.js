        function onSuccess(data, status,x)
        {
           ...
           if (x.responseText.indexOf("<docto>") != -1)
           {
                   ...
                   $a=$.parseXML(des);
                   var $b=$($a) 
                   co=$b.find("AvisoDeTestamento").attr("cadenaOriginal");
                   x = new Blob(des.split("\n"),{ type: 'application/xml'} );
                   fi = new fiel();
                   firmado=fi.firmastring(x);
                   $b.find("AvisoDeTestamento").attr("sello",firmado);
                   $b.find("AvisoDeTestamento").attr("certificado",cerg);
                   x = new Blob((new XMLSerializer()).serializeToString($b[0]).split("\n"),{ type: 'application/xml'} );
...
function fiel()
{
  ...
  this.validafiellocal = function(pwd)
  {
       ...
       try { pkg=ku.getKeyFromEncryptedPKCS8PEM(pk,pwd);} catch (err) {
             if(err.indexOf("code:001")) { alert ('El password no corresponde con la llave privada'); return false;} else {
             return err; return false;}} ;
       try {firmado=pkg.signString('prueba','sha512');} catch(err) { alert(err); return false; }
       cerg=localStorage.getItem('cer').substring(localStorage.getItem('cer').indexOf('base64,')+7);
       cer="-----BEGIN CERTIFICATE-----"+cerg.chunkString(64)+"-----END CERTIFICATE-----";
  ...
  this.firmastring = function (str)
  {
      if(pkg==null)
      { alert('No esta validada la llave privada'); return false; }
      return pkg.signString(str,'sha512')
  }
