...
function fiel()
{
  ...
  {
        return this.rk.signString(cadena,'sha512');
  }
  ...
  this.validafiellocal = function(pwd)
  {
       ...
       try {this.rk=ku.getKeyFromEncryptedPKCS8PEM(pk,pwd);} catch (err) {
             if(err.indexOf("code:001")) { alert ('El password no corresponde con la llave privada'); return false;} else {
             return err; return false;}} ;
       try {firmado=this.rk.signString('prueba','sha512');} catch(err) { alert(err); return false; }
