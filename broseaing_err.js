...
function que_cambio(wlmenu,wlmovto,wlllave,wlrenglon,event,nr,nc)
{
			...
			if (event.button==1)
	{
		mantto_tabla_1(wlmenu,"u",wlllave,wlrenglon);
	}
	else
	{
...
function muestra_cambio(theFormName,r,ct,wlllave,menu,wleventoantes,wleventodespues,wleventoantescl,wleventodespuescl,movto,noconfirmamovtos) {	
  ...
  pone_unchecked(r);
  wlonclick="mantto_tabla_1(\""+menu+"\",\"u\",\""+wlllave+"\","+(r)+",\""+wleventoantes+"\",\""+wleventodespues+"\",\""+wleventoantescl+"\",\""+wleventodespuescl+"\",\""+noconfirmamovtos+"\",this);return false;";
...
function CargaXMLDoc() 
{
       ...
       if (window.ActiveXObject)
       {
		...
		else
		{
    		if (window.XMLHttpRequest) 
    		{
        		...
        		req.open("POST", wlurl, true);  // sincrona
                        req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=UTF-8;");
        		req.send(passData);
                        req.onloadend = function() { querespuesta(); };
    		}
    	}
	}
	catch(err)
	{
                req='';
		alert('error en CargaXMLDoc'+err.message+' wlurl'+wlurl+' '+passData);
	}
}
...
function mantto_tabla_1(wlmenu,wlmovto,wlllave,wlrenglon,wleventoantes,wleventodespues,wleventoantescl,wleventodespuescl,noconfirmamovto,obje)
{
     var classante=obje.className;
     try {
           obje.className="lee";
           mantto_tabla(wlmenu,wlmovto,wlllave,wlrenglon,wleventoantes,wleventodespues,wleventoantescl,wleventodespuescl,noconfirmamovto,obje);
           obje.className=classante;
     } catch (err) {
           obje.className=classante;
     }
}
		...
		if (wlmovto=='S')
			{
	           ...
	           window.close();
        		...
        		wlurl='xmlhttp.php';//20071105
        		passData='&opcion=mantto_tabla&idmenu='+wlmenu+'&movto='+wlmovto+buildQueryString('formpr')+"&wlllave="+escape(wlllave)+"&wlrenglon="+escape(wlrenglon)+"&wleventodespues="+escape(wleventodespues)+"&filtro="+escape(armaFiltro('formpr'))+"&noconfirmamovto="+escape(noconfirmamovto);
	        	CargaXMLDoc();			
	        	return;
}
