...
//	if (empty($_GET['fecharecibo'])) {echo "La fecha de recepcion no esta definido"; die;}
        ...
        ##if ($num==0) {echo "No hay movimiento a imprimir las boletas".$me->camposm["fuente"]; die;}
...
$wldesasunto=substr($row['id_cveasunto'],0,strpos($row['id_cveasunto'],"="));
...
$wlasunto="";
      foreach ($me->camposmc as $index => $val)
      {
       if ($index!="fecharecibo" && $index!="id_cveasunto" && $index!="folioconsecutivo"  && $index!="" && $index!="fecha_alta" && $index!="usuario_alta"  && $index!="apepat" && $index!="apemat" && $index!="val_lc"  && $index!="folio" && $row[$index]!="" && $index!="nombre" && $index!="idsexo"  && $index!="edad"  && $index!="estatus"  && $index!="diastermino" && $index!="")
       {
          ...
          if ($row[$index]=='f')  { $row[$index]="NO"; }
          if ($row[$index]=='t')  { $row[$index]="SI"; }
          $wlasunto.=$val["descripcion"]." <vb>".(strpos($row[$index],"=")===false ? $row[$index] : substr($row[$index],0,strpos($row[$index],"=")))."</vb>;";
       }
      }
      $wlasunto="<p>".$wlasunto."</p>";
//$wlasunto=$row['asunto'];
...
$pdf->SetLineWidth(0.2);
  ...
  $black    = '000000'; // color in hexa
	...
	$pdf->SetFont('Arial','B',8);
        ...
        Barcode::rotate(-$len / 2, ($data['height'] / 2) + 8 + $marge, $angle, $xt, $yt);
	...
	$pdf->cell(0,4/$pix,$titulo4,0,1,'L',0);
        ...
        $data = Barcode::fpdf($pdf, $black, 15, $pdf->GetY()+$y+25 , 90, $type, array('code'=>$code), .4, $height);
        Barcode::rotate(-$len / 2, ($data['height'] / 2) + $fontSize + $marge, $angle, $xt, $yt);
        $pdf->TextWithRotation(24  , $pdf->GetY()+y+40 , $data['hri'], 90);
	...
	//$pdf->multiCell(0,$wlalto2/$pix,$wlasunto,'BR','C',1);
        $pdf->WriteTag(0,$wlalto2/$pix,$wlasunto,2,'J',1,0);
	global $parametro1;
