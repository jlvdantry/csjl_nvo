...
function marcallamado(id)
{
    ...
    try {
        ...
        passData='&opcion=llamaturno&wl_idgrupo='+wlmodulo;
        CargaXMLDoc(id);
...
function creallamado(wlfila,wlturno,wlgrupo,llamados,posicion,wlcomentario)
{
        ...
        //fila.setAttribute("onclick","llamaturno")
        ...
        //turno.setAttribute("onclick","llamaturno")
    ...
    if (req.readyState == 4)
    {
        ...
        if (req.status == 200)
        {
           ...
           if (req.responseText.indexOf("Encontro cita") != -1)
           {
              ...
              try { wlgrupo=items[0].childNodes[0].nodeValue; } catch (err) { };
              var items = req.responseXML.getElementsByTagName("_comentarios_");
              try { wlcomentarios=items[0].childNodes[0].nodeValue + " "; } catch (err) { };
              //creallamado(wlfila,wlturno,wlgrupo);
              msgx = { "fila" : wlfila, "turnogrupo" : wlturno, "desmodulo" : wlgrupo, "comentarios" : wlcomentarios };
              llamados=llamados+1;
              arr.push(llamados);
