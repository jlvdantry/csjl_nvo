...
Menu.prototype.closeTimeout = 10;
...
MenuBar.prototype.toHtml = function () {
	...
	for (var i = 0; i < l; i++ )
		itemsHtml[i] = items[i].toHtml();
//	alert('rows='+top.document.getElementById('fs').rows);
...
//	alert('tohtml'+this.toHtml());
	var el = this._document.getElementById( this.id );
//	alert('id='+el.id+' className='+el.className+' innerHTML='+el.innerHTML+' outerHTML='+el.outerHTML);
	this.hookupMenu( el );
};
...
//			window.status=window.status+"sht2";
		},
		...
		onclosetimer:	function ( id ) {
			...
			var sm = oThis.subMenu;
			sm.close();
//			window.status=window.status+"ct";			
		},
		...
		onpositionsubmenutimer:	function ( id ) {
			...
			if (veces<20)
			{ oThis.positionSubMenu(); veces=veces+1; sm.setSelectedIndex( 0 ); }
			else
			{ veces=0; }
//			alert("opsm"+id);
//			this._document.write("opsm"+id+";");
			
//			window.status=window.status+"opsm"+id+";";
		}
	},
	...
	menuBar:	{
		...
		ongotonextmenuitem:	function ( id ) {
			...
			mi.dispatchAction();
//			window.status=window.status+"nmit";			
			if ( mi.subMenu )
				mi.subMenu.setSelectedIndex( 0 );
		...
		ongotopreviousmenuitem:	function ( id ) {
			...
			var mi = oThis.items[ oThis.getSelectedIndex() ];
			mi.dispatchAction();
//			window.status=window.status+"pmit";						
			if ( mi.subMenu )
				mi.subMenu.setSelectedIndex( 0 );
		}
	},
