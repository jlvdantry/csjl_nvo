...
function sellado($pregunta) {
        ...
        $ruta = "" ;
        $file=$ruta."privadajlv.key";      // Ruta al archivo
        $llavepr=file_get_contents($file);
        debugea('llave privada='.$llavepr);
        // Obtiene la llave privada del Certificado de Sello Digital (CSD),
        $pkeyid = openssl_pkey_get_private($llavepr,'dantry');
        ...
        $file=$ruta."certjlv.cer";      // Ruta al archivo de Llave publica
...
function checarsellado($pregunta) {
        ...
        //$llavepr=$pregunta["certificado"];;
        //debugea('llave privada='.$llavepr);
        //$llavepr=file_get_contents("certjlv.cer");
