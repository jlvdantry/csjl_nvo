...
class PDF_WriteTag extends FPDF
{
	...
	var $Length; // URL for a cell
        var $Lineas; // Lineas del paragrafao si no alcaza a todas las llenas con un caracter

	// Public Functions

	function WriteTag($w, $h, $txt, $border=0, $align="J", $fill=false, $padding=0,$lineas=0)
	{
		$this->wLine=$w;
		$this->hLine=$h;
		$this->Text=trim($txt);
		...
		$this->BorderTop();
                $li=0;
		while(trim($this->Text)!="")
		{
                        $li++;
			$this->MakeLine();
			$this->PrintLine();
                        ##$this->Cell(0,14,"Linea=".$li." De lineas=".$lineas,1,0,'C');
		}
                       #$this->Cell(0,14,"Linea=".$li." De lineas=".$lineas,1,0,'C');
	...
	function MakeLineFill()
	{
                ...
                ##$i=$this->BeginLine();
                $this->TagName=array();
                ##$this->Cell(0,14,"Entro el fill".$lineas,1,0,'C');
 
 

                                   $i=0;
                                   while(($this->Length+$this->GetStringWidth($tab[2]))<$this->wTextLine)
                                   {
                                       $this->StringLength[$i]=$this->GetStringWidth($tab[2]);
                                       $this->Length+=$this->StringLength[$i];
                                       $this->LineLength[$i]=$this->Length;
	...
	function MakeLine()
	{
		...
		if($i==0) {
			$this->Length=$this->StringLength[0];
			$this->TagName[0]=1;
			$this->TagHref[0]=$this->href;
		}

                ##error_log($today." this->Text".$this->wTextLine."\n",3,"WriteTag_x01.log");
		while($this->Length<$this->wTextLine)
		{
			$tab=$this->Parser($this->Text);
			$this->Text=$tab[0];
                           $t=getdate();
                        $today=date('Y-m-d h:i:s',$t[0]);
                        ##error_log($today." Entro WriteTag_ tab0=".$tab[0]." tab1=".$tab[1]." tab2=".$tab[2]." this->Length=".$this->Length.">".$this->wTextLine."\n",3,"WriteTag_x01.log");

			if($this->Text=="") {
                                ##error_log($today." Entro en lastline\n",3,"WriteTag_x01.log");
				$this->LastLine=true;
				break;
			}
			...
			if($tab[1]=="o") {
				...
				if($this->TagStyle[$tab[2]]['indent']!=-1) {
					$this->Length+=$this->TagStyle[$tab[2]]['indent'];
			...
			if($tab[1]=="t")  {
				...
				$this->TagHref[$i]=$this->href;
                                if ($tab[2]=="--") {
                                   ##if ($this->Length>$this->wTextLine) {
                                   ##    $this->Length=$this->GetStringWidth($tab[2]);
                                   ##}
                                   while(($this->Length+$this->GetStringWidth($tab[2]))<$this->wTextLine)
                                   {
                                       $i++;
		...
		trim($this->Text);
		if($this->Length>$this->wTextLine || $this->LastLine==true)
			$this->EndLine();
	}
	...
	{
		$border=0;
		if($this->border>0 && $this->border<5)
			$border="LR";
                if($this->border==5)
			$border="L";
                if($this->border==6)
		...
		$this->DoStyle(0);
                ##if ($this->LastLine)
		##    $this->Cell($space,$this->hLine,"Ultima linea".count($this->Line2Print),0,0,'C',false,$href);
                $wt=0;
		for($i=0; $i<count($this->Line2Print); $i++)
		{
			...
			if($this->Line2Print[$i]=="")
				...
				$this->Cell($this->StringLength[$i],$this->hLine,$this->Line2Print[$i],0,0,'C',false,$href);
                        ...
                        $wt=$wt+$this->StringLength[$i];
		}
                ##if ($this->LastLine)
		##    $this->Cell($this->wLine-$wt,$this->hLine,str_pad("",$this->wLine-$this->Length,"-"),0,0,'L',false,$href);
		    ##$this->Cell($this->wLine-$wt,$this->hLine,"wTextLine=".$this->wTextLine." Length=".$this->Length,0,0,'L',false,$href);

		$this->LineBreak();
		if($this->LastLine && $this->Text!="") {
		        $this->Cell(0,$this->hLine,"",0,0,'C',false,$href);
			$this->EndParagraph();
                }
		$this->LastLine=false;
	}
