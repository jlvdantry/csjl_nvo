...
include("conneccion.php");
	...
	//echo "<textarea>$ventanilla</textarea>";
	...
	$sql =	"	select	".($foliosunicos=="t" ? "distinct" : "" )." date(fecha_altat) as fecha_altat,folio,".
			...
			"	".($foliosunicos=="t" ? ($puesto=="" ? "'-'" : "area" ): "area" )." as area	".
...
function Header()
{
    ...
    $this->Cell(0,6,"CONTROL DE FOLIOS".(($iterno=="t") ? " INTERNOS" : "" )." TURNADOS".($foliosunicos=="t" ? " (Sin detalle de turnos)" : " (Con detalle de turnos)" ),0,1,'C',0);
...
for ($i=0; $i<$num; $i++)
{
	...
	$pdf->Ln(1);
	//if ($e==25 && $num=25) {$e=0; $pdf->AddPage();}
	if ($pdf->GetY()>170 && $i+1<$num)
	{
		$pdf->AddPage();
		$pdf->SetFont('Arial','BI',$wlsize2);
		$pdf->Cell(40,$wlalto1,'Folios turnados: '.$row['fecha_altat'],0,1,'L',0);
