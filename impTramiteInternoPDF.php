...
include("conneccion.php");
	...
	$sql="	select vtr.*, vt.nombre_completo,vt.descripcion, vt.observacion, vt.liberado, vt.fecha_alta as falta, vt.usuario_alta as ualta, vt.liberado,	".
		 "	(select length(ficheroin) from contra.ope_archivos_interno as a where a.folioconsecutivo=vtr.folioconsecutivo and id_tipoarc=1 order by fecha_alta asc limit 1) as adjuntos	".
		 "	from contra.v_tramites_interno as vtr	".
		 "	LEFT JOIN contra.v_turnados_interno vt ON vt.folioconsecutivo = vtr.folioconsecutivo	".
...
function Header()
{
    ...
    $this->SetFont('Arial','B',9);
	$this->Cell(0,6,'CONTROL DE TRAMITES INTERNOS','B',1,'C',0);
...
function Footer()
{
	...
	$this->SetX($wlsetx);
	$this->SetFont('Arial','B',$wlsize2);
	$this->Cell(10,$wlalto2,'',0,0,'C',0);
	$this->Cell(100,$wlalto2,$firma_n,'T',0,'C',0);
	$this->Cell(10,$wlalto2,'',0,0,'C',0);
	...
	$this->SetX($wlsetx);
	$this->SetFont('Arial','B',$wlsize2);
	$this->Cell(120,$wlalto2,$firma_p,0,1,'C',0);
