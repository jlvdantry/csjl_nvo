...
$pix=.35;
if ($argv[1]!='') {
   $_GET['id_cveasunto']=$argv[1];
}
if ($argv[2]!='') {
   $_GET['wl_folioconsecutivo']=$argv[2];
}
	// Genera titulos
...
$pdf->SetLineWidth(0.2/$pix);
  ...
  $black    = '000000'; // color in hexa
	...
	$pdf->Image('img/cdmx_03.png',$wlsetx/$pix,11/$pix,20/$pix,0);
...
//	$pdf->Ln(1);
 $data = Barcode::fpdf($pdf, $black, $x, $y, $angle, $type, array('code'=>$code), $width, $height);
  ...
  $pdf->TextWithRotation($x + $xt, $y + $yt, $data['hri'], $angle);
    ...
    $pdf->SetFont('Arial','B',9);
	$pdf->SetX($wlsetx/$pix);
	$pdf->Cell(215/$pix,6/$pix,'BOLETA DE INGRESO A TRAMITES','B',1,'C',0);
	...
	##$pdf->Cell($wllargo2b/$pix,18/$pix,$wlfecharecibo,'BR',0,'C',1);
	...
	//$pdf->multiCell($wllargo2/$pix,$wlalto2/$pix,$wlasunto,'BR','C',1);
        $pdf->WriteTag($wllargo2/$pix,$wlalto1/$pix,$wlasunto,0,'J',0,0);
	global $parametro1;
