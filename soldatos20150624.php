...
*
       ...
       // 			? $table_width : "100" )."%; height:".(($table_height!="") ? $table_height : "100" )."%";
        ...
        if ($_SESSION["parametro1"]!="")
        { 
             echo "<td class='enca' ><input class='enca' align='left' type=image id='imostrar' src='images/xp/Lminus.png' ".
...
*/
        ...
        ($_SESSION["parametro1"]!=""
        ?	"		<td class='enca' >".
			"			<input class='enca' type=image id='imostrar' src='images/xp/Lminus.gif' onclick=\"return toggleDiv('".$this->titulos."',this);\" /></input>".
			"		</td>".
			"		<td class='enca' >".
...
//  20070523		else
//  20070523		{
    		return "<TABLE id='tabdinamica' class='scrolling_table_body'".
...
//20070818   Se incluyo el esquema en los campos select	            
//20070818	            " from ".$this->menuc[$wlcampo[0]]["fuente"].
...
//20080119     	 || (strpos($movtos,"S")!==true & $busqueda=='t')		 // 20070214
		 ...
		 ) 
		{
			...
			$wltdd="Seleccionar opcion";
   		    ...
   		    $vas="<td id='wlt_".$wlnombre."' name='wlt_".$wlnombre."' abbr=\"".$descripcion."\" value=\"".$descripcion."\" ".
...
##   			$vas=$vas."<td > ";
	        $vas=$vas.($wltdf=="1" || $wltdf=="2" || $wltdf=="3" ? "" : "<td>");
	 		$vas = ($menuc["autocomplete"]=='1' ? $vas." <input type=text readonly=true class='leesinborder' name=au_".$wlnombre." ></input><br>\n" : $vas );
   			$vas=$vas."<select placeholder=\"prueba\" onBlur='ponCookie(this.name,this.value);".
...
//			if ($fuente_campofil=="")  // si tiene el campo padre este se debe de llenar cuando se seleeciona el campo padre
			...
			{   	       
				...
				if ($num!=1 || ($obligatorio!=='t'))
	   			 $DefaultSelect="<option selected value >".(($wltdf==3) ? $wltdd : "Seleccione opcion" )."</option>\n"; // 20070622
     			for ($i=0; $i < $num ;$i++)
     			{
         			$Row = pg_fetch_array($sql_result, $i);
         			$a = trim($Row[1]);
...
// 20070622     		$vas=$vas."  </select>";  //20070214   		
			$vas=$vas.$DefaultSelect.$opciones."  </select>".$wlobligatorio;
	 		$vas = ($altaautomatico=='t' ? $vas. //20070214 	           		
             	  " <input type=image class=img src='img/add.gif' title='Alta de un nuevo registro de: ".$descripcion."' value='Alta' id='aa_".$wlnombre."' name=matriz ". 
	 		...
	 		$vas = ($fuente_busqueda=='t' ? $vas. //20070618
             	  "<input class=img type=image src='img/action_search_20.gif' title='Busca opcionessss' value='Busca' name=matriz value='Busca' ". //20070618
...
//20070611     if (strpos($md->camposm["movtos"],"u")!==false) 
     ...
     {
             $tt .= "<th id='cambiotxt' name=noimprime >Selec cionar</th>\n";                          
     }     
...
##  function arma_input ($nombre,$size,$male,$obligatorio,$descripcion,$tipodedato,$busqueda,$tcase,$j,$readonly,$valordefault,$espassword)  //20070214
  ...
  { 
	  ...
	  $wltdd="Capturar ".$mecq["descripcion"];
...
//	     echo " vas=".$nombre."=".$mecq;
    ...
    $wltipodedato=((substr($tipodedato,0,3)=='int' || $tipodedato=='numeric') ? " type='tel' " : "");
		$wli="<td <td id='wlt_".$nombre."' name=wlt_".$nombre." title=\"".$descripcion."\" abbr=\"".$descripcion."\"".
			(($wltdf=="1")
				?    " colSpan=2  "
			  : (($wltdf=="2")
			  	 ? " colSpan=4 "
	    ...
	    $wli=$wli.($wltdf=="1" || $wltdf=="2" || $wltdf=="3" ? ""  : "<td>").
	    				(($tipodedato != "text") 
	    					? "<input onKeydown='return quitaenter(this,event)' " 
	    					: "<textarea ".((preg_match("/1|2|3/",$wltdf) && strlen($mecq["colspantxt"])>0) 
...
*/
	    				(($tipodedato == "text") 
	    					? ($mecq["rowspantxt"]=="" 
	    						...
	    						: " ").
	                    (($male!="" && $male!="0") ? " maxlength=".$male : "" ).
                                (($readonly=='t') ? (($tipodedato == 'bool') ? " disabled=true " : " readonly=true ")."class='lee' " : " ").
                                (($valordefault=='' || $valordefault=='null') ? " " : (($valordefault=='hoy') ? " value=\"".date("Y")."-".date('m')."-".date("d")."\" " : " value=\"$valordefault\" " ) ). // grecar lo modifique para que cuando contega la palabra hoy, muestre la fecha del sistema
	    	            ($tip!="" ? " title='".$tip."'" : " ").
						...
						$wli=$wli.($wlonkc !="" ? " onClick='".$wlonkc.";'" : "" );																		
	    	            $wli=$wli.(($wltdf==3) ? " placeholder=\"".$wltdd."\"" : "").
	    	            (($tipodedato != "text") ? " ></input>" : " >".$valordefault."</textarea>").$wlobligatorio.
	    	            (($tipodedato == "timestamptz" || $tipodedato == "date") & $readonly!='t' ? " <input tabindex='-1' class='img' type=image id='fe_".$nombre."' name=fe_".$nombre." src='img/icon_datepicker.gif' onclick='muestrafecha(this);return false' title='Selecciona la fecha del calendario'></input>" : " " ).	    	            
                            (($espassword=="3") ? " <input tabindex='-1' size=14 class='captcha' readonly='on' type=text id='wl_".$nombre."_img' name=wl_".$nombre."_img title='Imagen de la captcha' ></input><input tabindex='-1' class='img' type=image id='wl_".$nombre."_bot' name=wl_".$nombre."_bot src='img/refresh.png' onclick='ReDrawCaptcha(this);return false' title='Refresca la imagen del captcha'></input>" : " " ).
	    	            (($tipodedato == "text") & $readonly!='t' ? " <input type=image class='img' id='txt_".$nombre."' name=txt_".$nombre." src='img/nota.gif' onclick='muestratexto(this);return false' title='Amplia el panel de la captura'></input>" : " " );  // 20070301  modificacion para abrir un ventana auxiliar en textos largos
...
// 20060117	            	&& $this->menuc[pg_fieldname($sql_result, $j)]["busqueda"]==t)
          ...
          { 
     	  ...
     	  echo "<td class='botones' > <input type=image class=img src='img/action_search_20.gif' title='Busca1' value='Busca' name=busca ".
...
##														,$nomcampo    // ups error duplicado, de acuerdo a la linea anterior debo apuntar a nomcampo
	          		...
	          		else
	          		{
		          		...
		          		{
           					$wllinea=$wllinea."<td>".$this->menuc[pg_fieldname($sql_result, $j)]["descripcion"]."</td>\n";          
            		  		$wllinea=$wllinea."<td> <input ".$this->armaid_cc($j)." type=text readonly=true name=wl_".pg_fieldname($sql_result, $j)."></input></td>\n";			          		
		          		}*/
		          		if ($this->menu["tiene_pk_serial"]==$nomcampo)  // si se cumple el campo es un serial
		          		{
			          		/// inicio de etiquta td
			          		$wllinea=$wllinea."<td ".
			          		/// segun el formato td define el paraetro colspan
			          		(	($wltdf==1)
			          			? " colSpan=2 " :	(	($wltdf==2 || $wltdf==3)
			          									? " colSpan=100 " : ""
			          								)
			          		)
			          		/// descripcion de la etiqueda td pk
			          		.">".(($wltdf==3) ? "" : $wltdd ).	
			          		(	($wltdf==1 || $wltdf==2)
			          			? " <br><input  " : (($wltdf==3) ? " <input  " : "</td><td><input ")  
			          		).
	            		  	//$wllinea=$wllinea."<input ".
	            		  	...
	            		  	(($wltdf==3) ? "placeholder=\"".$wltdd : "" )."\"></input></td>\n";
	            		  	//echo $wllinea;
            		  	}
		          		else
		          		{
				...
				if ($this->menuc[$nomcampo]["htmltable"]!=$htmltableanterior)
				{
					...
					if ($htmltableanterior!="0") { echo "</div $htmltableanterior>"; }			    			
	    		                ...
	    		                $this->inicio_tabcaptura_t($this->menu["table_width"],$this->menu["table_height"],$this->menu["table_align"]);	
                                        echo "<tr><td class=htmltable colspan=".($this->menu["columnas"]*2).">";
                                        echo "<input class='enca	' align='left' type=image id='imostrar' src='images/xp/Lminus.gif' ".
...
//20070920     		echo "<td  class='botones' > <input type=image id='iAlta' src='img/add.gif' title='alta' value='Alta' name=matriz ".
            ...
            ($this->menum['i']['idmovto']=='i' && $this->menum['i']["descripcion"]!="" ? "<input type=button id='iAlta' value='".$this->menum['i']["descripcion"]."' title='Alta'  " : "<input type=button id='iAlta' src='img/add.gif' title='Alta' value='Alta' name=matriz ").  //20070920
...
//20071112     se incluyo los evento que se efectuar en el cliente        	                          
        ...
        {
     	echo "<td class='botones'> <input type=button id='iBusca' src='img/action_search_20.gif' title='Buscat'  name=busca value=Busca ".
##        	  "onclick='abre_consulta(\"".$this->idmenu."\");return false'></input></td>\n";	        
        	"onclick='mantto_tabla(\"".$this->idmenu.
        	                          "\",\"".
...
//        	 "  onclick=\"x()\" value=\"charros\" ></input></td>\n";	        
			echo "<TD id='imprime' class=botones > <input onclick='imprime(\"\");return false' id='iimprime'  value=Imprime type=button></input> </TD>";
        }
        		
        if (strpos($this->movto_mantto,"l")!==false)     	  
        {
     	    echo "<td id='Limpiar' class='botones' > <input type=button id='iLimpiar' title='Limpia datos de la pantalla'  ".
        	 "  onclick='pone_focus_forma(\"formpr\");formReset(\"formpr\",\"t\")' value='Limpiar' ></input></td>\n";	        
        }

        if (strpos($this->movto_mantto,"f")!==false)     	  
        {
     	    echo "<td id='Archvio' class='botones' > <input type=button id='iArchvio' title='Genera archvio en txt'  ".
        	"onclick='mantto_tabla(\"".$this->idmenu.
        	                          "\",\"f\",\"\",\"\",\"".
        	                          // evento antes 
        	                          (($this->menue[9][2][donde]==1) ? $this->menue[9][2][descripcion] : "").
        ...
        {
            echo "<td id=Excel class='botones' > <input type=button id='iExcel' title='Genera archvio en Excel'  ".
                "onclick='mantto_tabla(\"".$this->idmenu.
                                          "\",\"ex\",\"\",\"\",\"".
                                          // evento antes
                                          (($this->menue[9][2][donde]==1) ? $this->menue[9][2][descripcion] : "").
...
//20070223  oh este dia fue muy productivo, inclui la opcion a que es autodiseñ/20070223  esto es muy util cuando aun estas ajustando la forma                
        if (strpos($meda->camposm[movtos],"a")!==false) 
        	{
				echo "<td id='Autodiseno' class=botones ><input type=button id='iAutoDiseno' title='Entra al diseno de la forma' ".
...
##  20070226					$wl=$wl.$this->arma_td($z,$j,$Row[pg_fieldname($sql_result, $j)]); 
								...
								{
									$wl=$wl."<td ".$this->armaid_td($z,$j)."><a href=upload_ficheros/".$Row[pg_fieldname($sql_result, $j)]." >".$Row[pg_fieldname($sql_result, $j)]."</a></td>";
								}
...
//20070611     if (strpos($meda->camposm[movtos],"u")!==false) 
  ...
  {
			return "<td name=noimprime>".
				  (	$meda->camposmsv[$mm]['esboton']==1 ? "<input type=button " : "<a href=## ").
