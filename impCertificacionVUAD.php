...
function Header()
{
	...
	global $row;
	$this->SetTextColor(0);
	$this->Image('img/cdmx_03.png',20,16,40,0);
        $this->Ln(25);
        $enca='<p>VOL. <vb>'.$row['vol'].'</vb>                <vb>'.$row['des_tipoprotocolo'].'</vb>                  '.($row['id_tipo_autorizacion']=='4' ? '<vb>APENDICE</vb>' : 'FOJA <vb>'.$row['escr_hojac']).'</vb></p>';
        ...
        { 
          ...
          $wltexto='LA PRESENTE ES COPIA '.($row['id_tipocopia']==3 ? 'CERTIFICADA' : 'SIMPLE').' DEL INSTRUMENTO NOTARIAL NUMERO '; 
        ...
        {
          $wlcoteja='COTEJADO CON SU ORIGINAL MISMO QUE SE ENCUENTRA '; 
        ...
        if ($row['id_tipocopia']==2)
        {
          $wlcoteja='COTEJADO CON SU ORIGINAL MISMO QUE SE ENCUENTRA '; 
          $a= new NumberConverter();
          $wltexto='EL PRESENTE ES EL '.strtoupper($a->convert($row['drpp_primer'],"p")).' TESTIMONIO PARA EFECTOS DE INSCRIPCIÓN EN EL REGISTRO PÚBLICO DE LA PROPIEDAD '.strtoupper(utf8_decode($a->convert($row['drpp_orden'],"O"))).' EN SU ORDEN DEL INSTRUMENTO NOTARIAL NUMERO ';
          $wlfojast='EL PRESENTE TESTIMONIO VA EN ';
          $wlfojasut=" FOJAS ÚTILES ".$wlplanos." CON LOS REQUISITOS LEGALES";
          $wlfojas=numtoletras($row['fojas'],1);
        };
	...
	$this->WriteTag(0,$wlalto1,$wltexto,0,'J',0,0,4);
	...
	$this->WriteTag(0,$wlalto1,$wlfecha,0,'J',0,0,3);
        ...
        if ($row['id_tipo_autorizacion']=='3')
        {
          ...
          if ($row['apr_asociado']!='0' && $row['apr_asociado']!='')
          {
             $wlnota='<vb>'.numtoletras($row['apr_asociado']).'</vb>, LICENCIADO <vb>'.$row['apr_asociadonombre'].'</vb> ACTUANDO COMO ASOCIADO EN EL PROTOCOLO DEL NOTARIO <vb>'.numtoletras($row['apr_not']).'</vb>';
          }
          if ($row['apr_suplente']!='0' && $row['apr_suplente']!='')
          {
             $wlnota='<vb>'.numtoletras($row['apr_suplente']).'</vb>, LICENCIADO <vb>'.$row['apr_suplentenombre'].'</vb> ACTUANDO COMO SUPLENTE EN EL PROTOCOLO DEL NOTARIO <vb>'.numtoletras($row['apr_not']).'</vb>';
          }
          $wlnota.=($row['apr_fechaaut']!='' ? ', '.substr($row['apr_fechaaut'],8,2).' DE '.strtoupper(meses_espanol(intval(substr($row['apr_fechaaut'],5,2)))).' DE '.numtoletras(substr($row['apr_fechaaut'],0,4),1) : '');

          $wlencuentra_a='<vb> Y AUTORIZADO DEFINITIVAMENTE</vb> POR EL NOTARIO NUMERO ';
          if (($row['ade_asociado']=='0' || $row['ade_asociado']=='') && ($row['ade_suplente']=='0' || $row['ade_suplente']==''))
          {
             $wlnota_a='<vb>'.numtoletras($row['ade_not']).'</vb> DE LA CIUDAD DE MÉXICO, LICENCIADO <vb>'.$row['ade_nombre'].' --';
          }
          if ($row['ade_asociado']!='0' && $row['ade_asociado']!='')
          {
             $wlnota_a='<vb>'.numtoletras($row['ade_asociado']).'</vb>, LICENCIADO <vb>'.$row['ade_asociadonombre'].'</vb> ACTUANDO COMO ASOCIADO EN EL PROTOCOLO DEL NOTARIO <vb>'.numtoletras($row['ade_not']).' --';
          }
          if ($row['ade_suplente']!='0' && $row['ade_suplente']!='')
          {
             $wlnota_a='<vb>'.numtoletras($row['ade_suplente']).'</vb>, LICENCIADO <vb>'.$row['ade_suplentenombre'].'</vb> ACTUANDO COMO SUPLENTE EN EL PROTOCOLO DEL NOTARIO <vb>'.numtoletras($row['ade_not']).' --';
          }
          if ($row['id_cveasunto']==38) {   /* TEXTO PARA AUTORIZACION DEFINITIVA */
              $wlencuentra_a='<vb> Y AUTORIZADO DEFINITIVAMENTE</vb> POR EL SUSCRITO SUBDIRECTOR DE ARCHIVO GENERAL DE NOTARÍAS DE LA CIUDAD DE MÉXICO, MEDIANTE ACUERDO <vb>'.$row['folio'].'-'.substr($row['fecharecibo'],0,4).'</vb>';
              $wlnota_a='';
         }

          $wlnota_a.=($row['ade_fechaaut']!='' ? ' DEL '.substr($row['ade_fechaaut'],8,2).' DE '.strtoupper(meses_espanol(intval(substr($row['ade_fechaaut'],5,2)))).' DE '.numtoletras(substr($row['ade_fechaaut'],0,4)) : '');

        }

        if ($row['id_tipo_autorizacion']=='4')
        {
             $wlnota='<vb>'.numtoletras($row['ade_not']).'</vb> DE LA HOY CIUDAD DE MÉXICO, LICENCIADO <vb>'.$row['ade_nombre'].' --';
             $wlcoteja='COTEJADA CON SU LIBRO APENDICE DE COTEJOS DEL NOTARIO NUMERO ';
        }
        $this->Ln(1);
        $wlnota='<p>'.$wlcoteja.$wlencuentra.$wlnota.$wlencuentra_a.$wlnota_a.'</p>';
        $this->WriteTag(0,$wlalto1,$wlnota,0,'J',0,0,12); 
	$this->SetFont('Courier','',12);
        $wlparat='MISMO QUE OBRA DEPOSITADO EN EL ARCHIVO GENERAL DE NOTARÍAS DE LA CIUDAD DE MÉXICO, Y SE EXPIDE EN USO DE LA FACULTAD QUE ME CONFIEREN LOS ARTÍCULOS DOSCIENTOS TREINTA Y OCHO FRACCIONES V Y VI, DOSCIENTOS TREINTA Y NUEVE Y DOSCIENTOS CUARENTA Y CUATRO DE LA LEY DEL NOTARIADO VIGENTE PARA EL DISTRITO FEDERAL, COMO SUBDIRECTOR DE ARCHIVO GENERAL DE NOTARÍAS DE LA CIUDAD DE MÉXICO, TITULAR DEL MISMO ARCHIVO PARA: ';
	//$this->Justify($wlpara,200,10);
	//$this->WriteTag(0,$wlalto1,$wlparat,0,'J');
        $wlpara=$row['para'];
        $this->Ln(1);
        $wlpara='<p>'.$wlparat."<vb>".$wlpara."</vb>--</p>";
	$this->WriteTag(0,$wlalto1,$wlpara,0,'J',0,0,15);
	//$this->MultiCell(0,$wlalto1,$wlfojast,0,'J');
        $this->Ln(1);
        $wlpad="";
        $wlfojas='<p>'.$wlfojast."<vb>".$wlfojas."</vb>".$wlfojasut.$wlpad.'<vb></vb>--</p>';
	$this->WriteTag(0,$wlalto1,$wlfojas.$wlpad,0,'J',0,0,4);
}
...
function Footer()
{
	...
	$this->SetFont('Courier','',12);
        ...
        $this->Multicell(90,$wlalto1,$firman,1,'C');
