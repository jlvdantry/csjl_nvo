...
class  PdfToText 	// extends Object
   {
	...
	// Page mappings
	protected	$PageMappings		=  array() ;
	// Page locations - holds page numbers and start/end offsets in the Text property 
	protected	$PageLocations		=  array() ;
	...
	/*--------------------------------------------------------------------------------------------------------------
	    ...
	    NAME
	        AddPage - Adds a page reference to the page mappings.
	
	    PROTOTYPE
	        $this -> AddPage ( $object_data ) ;
	
	    DESCRIPTION
	        Normally, a way to distinguish pages is locating a /Type/Page construct. When this construct is present,
		there is a /Contents keyword that references the object who has the page contents.
	 ...
	 *-------------------------------------------------------------------------------------------------------------*/
	protected function  AddPage ( $object_id, $object_data )
	   {
		if  ( preg_match ( '#/Contents \s+ (?P<object> \d+)#imsx', $object_data, $match ) )
		   {
			$count						=  count ( $this -> PageMappings ) ;
			$this -> PageMappings [ $match [ 'object' ] ]	=  
			   array ( 
				'page'		=>  $count + 1, 
				'page-object'	=>  ( integer ) $match [ 'object' ],
				'object'	=>  ( integer ) $object_id
			    ) ;
		    }
	    }
	...
	/*--------------------------------------------------------------------------------------------------------------
	    ...
	    NAME
	        GetPageList - Retrieves the page list given by the /Pages/Kids[] construct.
	
	    PROTOTYPE
	        $list	=  $this -> GetPageList ( $object_data ) ;
	
	    DESCRIPTION
	        This method is called to catch the optional /Pages/Kids[] construct, that lists between square brackets
		the references to the objects that hold the successive page contents.
	    ...
	    RETURN VALUE
	        Returns an associative array whose keys are the page numbers and values the object ids.
	
	 *-------------------------------------------------------------------------------------------------------------*/
	protected function  GetPageList ( $object_data )
	   {
		if  ( preg_match ( '#/Kids \s* \[ (?P<list> [^\]]+)#imsx', $object_data, $list ) )
		   {
			preg_match_all ( '#(?P<object> \d+) \s+ \d+ \s+ R#imsx', $list [ 'list' ], $matches ) ;
			$page_number	=  0 ;
			$result		=  array() ;

			foreach ( $matches [ 'object' ]  as  $id )
				$result [ ++ $page_number ]	=  ( integer ) $id ;
	...
	/*--------------------------------------------------------------------------------------------------------------
	    ...
	    NAME
	        RemapPages - Remaps page information.
	
	    PROTOTYPE
	        $this -> RemapPages ( $pages ) ;
	
	    DESCRIPTION
	        Remaps the page numbers of the current document.
	
	    PARAMETERS
	        $pages (associative array) -
	                An array of page/object ids associations that has been returned by the GetPageList() method.
			If this parameter is empty, no remapping will occur.
	
	    NOTES
	        It may happen that the page list given by the /Pages /Kids[] provides a page order that is different
		from the sequential page order given by the /Page /Contents constructs. Although I didn't encounter
		such a situation so far, this method will renumber the page numbers built sequentially using the real
		order given by the /Kids option.
	
	 *-------------------------------------------------------------------------------------------------------------*/
	protected function  RemapPages ( $pages )
	   {
		if  ( ! $pages )
			return ;

		foreach  ( $this -> PageMappings  as  &$mapping )
		   {
	...
	/*--------------------------------------------------------------------------------------------------------------
	    ...
	    NAME
	        IsPage - Check if the specified object contents denote a page reference.

	    PROTOTYPE
	        $status		=  $this -> IsPage ( $object_data ) ;

	    DESCRIPTION
	        Checks if the specified object contents denote a page reference.
	    ...
	    RETURN VALUE
	        True if the specified contents contains a /Type/Page keyword, false otherwise.

	 *-------------------------------------------------------------------------------------------------------------*/
	protected function  IsPage ( $object_data )
	   {
		if  ( preg_match ( '#/Type\s*/Page[^s]#', $object_data ) )
			return(  true ) ;
	...
	/*--------------------------------------------------------------------------------------------------------------
	    ...
	    NAME
	        IsPageList - Check if the specified object contents denote a page list

	    PROTOTYPE
	        $status		=  $this -> IsPageList ( $object_data ) ;

	    DESCRIPTION
	        Checks if the specified object contents denote a page list.
	    ...
	    RETURN VALUE
	        True if the specified contents contains a /Type/Pages/Kids keyword, false otherwise.

	 *-------------------------------------------------------------------------------------------------------------*/
	protected function  IsPageList ( $object_data )
	   {
		if  ( preg_match ( '#/Type\s*/Pages\s*/Kids#imsx', $object_data ) )
			return(  true ) ;
		...
		while  ( ( $part = $this -> __next_token ( $data, $data_length, $index ) )  !==  false )
		   {
			...
			// If we are in a page header or footer definition, ignore evrything
			if  ( $ignore )  
			   {
				if  ( $token  ==  'ET' )
					$ignore		=  false ;
			...
			// 'Tm' instruction : return a "goto" instruction with the x and y coordinates
			else if  ( $token  ==  'Tm' )
			   {
				...
				return ( array( 'instruction' => 'goto', 'next' => $next_index, 'x' => $x, 'y' => $y ) ) ;
			    }
			// Output text "'" instruction, with conditional newline
			else if  ( $token [0]  ==  "'" )
				return ( array( 'instruction' => 'nl', 'next' => $next_index, 'conditional' => true, 'leading' => false ) ) ;
			// Same as above
			else if  ( $token  ==  'TJ'  ||  $token  ==  'Tj' )
				return ( array( 'instruction' => 'nl', 'next' => $next_index, 'conditional' => true, 'leading' => false ) ) ;
			// Set font size
			else if  ( $token  ==  'Tf' )
				return ( array( 'instruction' => 'fontsize', 'next' => $next_index, 'size' => $number_stack [0] ) ) ;
			// Text leading (spacing used by T*)
			else if  ( $token  ==  'TL' )
				return ( array( 'instruction' => 'leading', 'next' => $next_index, 'size' => $number_stack [0] ) ) ;
			...
			// Raw text output
			else if  ( $token [0]  ==  '(' )
			   {
			   	...
			   	$instruction	=  array( 'instruction' => 'text', 'next' => $next_index, 'values' => array( $token ) ) ;
			...
			else if  ( $token [0]  ==  '[' )
			   {
				...
				$instruction 	=  array( 'instruction' => 'text', 'next' => $next_index, 'values' => $values ) ;
			...
			// "/R" : Specifies the font to be used for the next text output(s)
			else if  ( ! strncasecmp ( $token, '/R', 2 ) )
			   {
				...
				return ( array( 'instruction' => 'resource', 'next' => $next_index, 'resource' => $resource ) ) ;
			    }
			...
			else if  ( ! strncasecmp ( $token, '/F', 2 ) )
			   {
				...
				return ( array( 'instruction' => 'resource', 'next' => $next_index, 'resource' => $id ) ) ;
			    }
...
/*==============================================================================================================
  ...
  ==============================================================================================================*/
abstract class	PdfTexterCharacterMap	// extends Object
					implements  \ArrayAccess, \Countable
   {
	...
	public static function  CreateInstance ( $object_id, $definitions )
	   {
		if  ( stripos ( $definitions, 'begincmap' ) )
			return ( new PdfTexterUnicodeMap ( $object_id, $definitions ) ) ;
		else if  ( stripos ( $definitions, '/Differences' ) )
		...
		if  ( preg_match_all ( '/ beginbfchar \s* (?P<chars> .*?) endbfchar /imsx', $definitions, $char_matches ) )
		    {
				...
				// I have met one example where all was put on the same line
	...
	public function  offsetGet ( $offset )
	   {
		if  ( isset ( $this -> Map [ $offset ] ) )
		   {
		   	$entity 	=  '&#' . sprintf ( "%d", $this -> Map [ $offset ] ) . ';' ;
...
/*==============================================================================================================
  ...
  ==============================================================================================================*/
class  PdfTexterPageMap		//extends  Object
   {
	public function  __construct ( )
	   {
