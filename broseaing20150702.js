			...
			if (wlalta==1)
			{
	  			if (opciones.length>0	)
	    			  { clearSelect(opciones); }   // si llega a esta instancia no encontro el dato por que lo se supones que es un select que depende de otro
        		        ...
        		        appendToSelect(opciones, '', document.createTextNode("Selecciona una Opcion"),2)      // 20070810  		        
...
function buildTopicList(wl,des,val) 
{
    ...
    appendToSelect(wl, "", document.createTextNode("Selecciona una Opcion"),(items.length==1 ? 0 : 1)); //   20070808    
    wl.click;
}
