...
function Header()
{
	...
	$this->Image('img/cdmx_02.png',10,14,0,0);
        ...
        { 
          ...
          $wlfojasut=" FOJAS ÚTILES CON LOS REQUISITOS LEGALES";
        ...
        $this->Ln(5);
        $wlescri=numtoletras($row['escr']);
        $wltexto='<p>'.$wltexto.'<vb>'.str_pad($wlescri,280-strlen($wltexto.$wlescri)," -").'</vb></p>';
	$this->WriteTag(0,$wlalto1,$wltexto,0,'J',0);
        ...
        $wlpad="";
        $wlfecha='<p>DE FECHA <vb>'.$wlfecha.'</vb>'.str_pad($wlpad,122-(9+strlen($wlfecha))," -").'</p>';
        //$wlfecha='<p>DE FECHA <vb>'.$wlfecha.'</vb></p>';
        $this->Ln(1);
	$this->WriteTag(0,$wlalto1,$wlfecha,0,'J');
	...
	//$this->WriteTag(0,$wlalto1,$wlcoteja,0,'J');
        ...
        { 
          ...
          {
             $wlnota='<vb>'.str_pad(numtoletras($row['apr_not']).'</vb> DE LA CIUDAD DE MÉXICO, LICENCIADO <vb>'.$row['apr_nombre'].'</vb>',263,' -');
          }
          if ($row['apr_asociado']!='0' && $row['apr_asociado']!='')
          {
             $wlnota='<vb>'.str_pad(numtoletras($row['apr_asociado']).'</vb>, LICENCIADO <vb>'.$row['apr_asociadonombre'].'</vb> ACTUANDO COMO ASOCIADO EN EL PROTOCOLO DEL NOTARIO <vb>'.numtoletras($row['apr_not']),263,' -');
          }
        ...
        { 
          ...
          {
             $wlnota='<vb>'.str_pad(numtoletras($row['ade_not']).'</vb> DE LA CIUDAD DE MÉXICO, LICENCIADO <vb>'.$row['ade_nombre'].'</vb>',263,' -');
          }
          if ($row['ade_asociado']!='0' && $row['ade_asociado']!='')
          {
             $wlnota='<vb>'.str_pad(numtoletras($row['ade_asociado']).'</vb>, LICENCIADO <vb>'.$row['ade_asociadonombre'].'</vb> ACTUANDO COMO ASOCIADO EN EL PROTOCOLO DEL NOTARIO <vb>'.numtoletras($row['ade_not']),263,' -');
          }
          if ($row['ade_suplente']!='0' && $row['ade_suplente']!='')
          {
             $wlnota='<vb>'.str_pad(numtoletras($row['ade_suplente']).'</vb>, LICENCIADO <vb>'.$row['ade_suplentenombre'].'</vb> ACTUANDO COMO SUPLENTE EN EL PROTOCOLO DEL NOTARIO <vb>'.numtoletras($row['ade_not']),263,' -');
          }
        }
        $this->Ln(1);
        $wlnota='<p>'.$wlcoteja.$wlencuentra.$wlnota.'</p>';
        $this->WriteTag(0,$wlalto1,$wlnota,0,'J'); 
	$this->SetFont('Courier','',14);
        $wlparat='MISMO QUE OBRA DEPOSITADO EN EL ARCHIVO GENERAL DE NOTARÍAS DE LA CIUDAD DE MÉXICO, Y SE EXPIDE EN USO DE LA FACULTAD QUE ME CONFIEREN LOS ARTÍCULOS DOSCIENTOS TREINTA Y OCHO FRACCION V Y VI, DOSCIENTOS TREINTA Y NUEVE Y DOSCIENTOS CUARENTA Y CUATRO DE LA LEY DEL NOTARIADO VIGENTE PARA EL DISTRITO FEDERAL, COMO SUBDIRECTOR DE ARCHIVO GENERAL DE NOTARIAS DE LA CIUDAD DE MÉXICO, TITULAR DEL MISMO ARCHIVO PARA: ';
	//$this->Justify($wlpara,200,10);
	//$this->WriteTag(0,$wlalto1,$wlparat,0,'J');
        $wlpara=$row['para'];
        $this->Ln(1);
        $wlpara='<p>'.$wlparat."<vb>".$wlpara."</vb>".str_pad($wlpad,251-(strlen($wlpara))," -")."</p>";
	$this->WriteTag(0,$wlalto1,$wlpara,0,'J');
	//$this->MultiCell(0,$wlalto1,$wlfojast,0,'J');
        $this->Ln(1);
        $wlpad="";
        $wlpad=str_pad($wlpad,120-strlen($wlfojast.$wlfojas.$wlfojasut)," -");
        $wlfojas='<p>'.$wlfojast."<vb>".$wlfojas."</vb>".$wlfojasut.$wlpad.'</p>';
	$this->WriteTag(0,$wlalto1,$wlfojas.$wlpad,0,'J');
}
...
function Footer()
{
        ...
        $this->SetY(-85);
	$this->SetFont('Courier','',14);
        $wlfechacerti='<p>'.str_pad('CIUDAD DE MÉXICO A <vb>'.numtoletras(substr($row['fecha_certificacion'],8,2)).' DE '.strtoupper(meses_espanol(intval(substr($row['fecha_certificacion'],5,2)))).' DE '. numtoletras(substr($row['fecha_certificacion'],0,4)).'</vb>',130,' -').'</p>';
        $this->Ln(1);
