...
{
    if(isset($escron))
    {	$connection = pg_connect("host=$wlhost dbname=$wldbname user='jlv' password='jlv' port=$wlport") or die("Error en el cron");	}
    else
    {	$connection = pg_connect("host=$wlhost	dbname=$wldbname user='temporal' password='Temporal1' port=$wlport") or die("Error con clave temporal");	
        pg_exec($connection,"set client_encoding to 'latin1'");
    }
}
?>
