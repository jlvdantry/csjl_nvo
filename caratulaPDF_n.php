...
$wldesasunto=$row["desasunto"];
        ...
        {
          $wldesasunto='LA PRESENTE ES COPIA '.($row['id_tipocopia']==3 ? 'CERTIFICADA' : 'SIMPLE').' DEL INSTRUMENTO NOTARIAL NUMERO '.number_format($row["escr"]);
        };
        if ($row['id_tipocopia']==1)
        {
          $a= new NumberConverter();
          $wldesasunto=strtoupper($a->convert($row['tes_orden'],"O")).' TESTIMONIO '.strtoupper($a->convert($row['tes_sol'],"O")).' PARA EL SOLICITANTE DEL INSTRUMENTO NOTARIAL NUMERO '.number_format($row["escr"]);
        };
        if ($row['id_tipocopia']==2)
        {
          $a= new NumberConverter();
          $wldesasunto=strtoupper($a->convert($row['drpp_primer'],"O")).' TESTIMONIO PARA EFECTOS DE INSCRIPCIÓN EL EN REGISTRO PÚBLICO DE LA PROPIEDAD Y DE COMERCIO '.strtoupper(utf8_decode($a->convert($row['drpp_orden'],"O"))).' EN SU ORDEN DEL INSTRUMENTO NOTARIAL NUMERO '.number_format($row["escr"]);
        };

$wldiacaratula=$row["fecha_certificacion"];
...
function Header()
{
	...
	$this->SetX($wlsetx);
	$this->SetFont('Arial','B',$wlsize2);
	$this->multiCell($wllargo2,7,$wldesasunto,'BR','C',1);
