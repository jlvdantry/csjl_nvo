...
function Header()
{
	...
	$this->Line(70,8,70,28);
	$this->SetY(10);
	$this->cell(72,4,'',0,0,'L',0);
	...
	$this->cell(72,4,'',0,0,'L',0);	
	$this->cell(0,4,'ENTREGA DE AVISOS DE TESTAMENTO A LAS NOTARÍAS',0,1,'L',0);
	$this->Ln(8);
	...
	$num= pg_numrows ($sql_result);	
        $row=pg_fetch_array($sql_result,0);
        $row['gestor']=substr($row['gestor'],0,strpos($row['gestor'],"="));
	$this->SetFont('Arial','',9);
        $this->SetX(10);
	$this->Cell(10,$wlalto1,'Gestor:',0,0,'L',0);
	$this->Cell(50,$wlalto1,$row["gestor"],'LB',0,'L',0);
	$this->Cell(25,$wlalto1,'Fecha de entrega:',0,0,'L',0);
	$this->Cell(25,$wlalto1,$row["diaalta"],'LB',0,'C',0);
	$this->Cell(8,$wlalto1,'Folios:',0,0,'L',0);
	$this->Cell(8,$wlalto1,$num,'LB',1,'C',0);
	$this->Ln(3);
	$this->SetX(10);
	$this->SetFont('Arial','B',$wlsize1);
      $this->Cell(8,$wlalto1,'No.',0,0,'L',0);
      foreach ($me->camposmc as $index => $val)
      {
       if ($index!="fecharecibo" && $index!="id_cveasunto" && $index!="folioconsecutivo"  && $index!="" && $index!="fecha_alta"  && $index!="apepat" && $index!="apemat" && $index!="val_lc" && $index!="gestor" && $index!="diaalta" && $index!="id_tipotra"  && $index!="id_turnado" && $index!="observacion")
       {
...
for ($i=0; $i<$num; $i++)
{
	$row=pg_fetch_array($sql_result,$i);
        $row['testador']=html_entity_decode($row['testador']);
/*
...
//	$pdf->Cell(5,$wlalto2,$e,0,0,'C',0);
	$pdf->Cell(8,$wlalto2,$i+1,'BR',0,'C',1);
//	$pdf->Cell(1,$wlalto2,'',0,0,0,0);
      $wlnombre="";
      foreach ($me->camposmc as $index => $val)
      {
       if ($index!="fecharecibo" && $index!="id_cveasunto" && $index!="folioconsecutivo"  && $index!="" && $index!="fecha_alta"  && $index!="val_lc" && $index!="gestor" && $index!="diaalta" && $index!="id_tipotra"  && $index!="id_turnado" && $index!="observacion")
       {
          $pon=$row[$index];
