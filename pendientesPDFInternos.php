...
include("conneccion.php");
	...
	$sql =	"	select	".
			...
			"	count (*) as reg	".
			"	from contra.v_pendientes_interno vp	".
			...
			"	where vp.id_organizacion=(select id_organizacion from contra.cat_personas where usename=current_user) and vp.respuesta is true	".
			"	and liberado='N'	".
			"	$wlfltro ".
			"	group by 1,2,3,4	";
	$sql_result = pg_exec($connection,$sql);
	if (strlen(pg_last_error($connection))>0) { echo "Error al ejecutar qry ".$sql." ".pg_last_error($connection); }
	$num= pg_numrows ($sql_result);	
	if ($num==0) {echo "No existen pendientes"; die;}
	...
	$sql =	"	select vp.*,case when relevante is true then 'Si' else 'No' end as rv,	".
			...
			"	from contra.v_pendientes_interno as vp	".
			"	left join contra.cat_organizaciones as co on co.id_organizacion=vp.id_organizacion	".
			"	left join contra.cat_puestos as cp on cp.id_puesto=vp.id_puesto	".
			"	left join contra.cat_tipo_tramite as ctt on ctt.id_tipotra=vp.id_tipotra	".
			"	left join contra.v_cat_personas as cpe on cpe.id_persona=vp.id_persona	".
...
function Header()
{
	...
	$this->Cell(0,6,'REPORTE DE PENDIENTES INTERNOS','B',1,'C',0);
	$this->Ln(3);
}
...
$pdf->Cell(50,6,'Pendientes'.($relvante=='t' ? ' relevantes, ' : ', ').'recibidos entre '.$fecharecibo.' y '.$fecharecibofin,'B',0,'L',0);
$pdf->Cell(0,6,'Hoja con resumen','B',1,'R',0);
$pdf->Ln(2);
...
$pdf->Cell(50,6,'Pendientes'.($relvante=='t' ? ' relevantes, ' : ', ').'recibidos entre '.$fecharecibo.' y '.$fecharecibofin,'B',0,'L',0);
$pdf->Cell(0,6,'Hoja con detalle','B',1,'R',0);
$pdf->Ln(1);
$pdf->SetFont('Arial','I',$wlsize1);
